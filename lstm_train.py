...
class LSTMNER:
    ...
    def modify_data(self):
        ...
        ran_list = [i for i in range(len(x_train))]
        random.shuffle(ran_list)
        ran_x_train = [x_train[i] for i in ran_list]
        ran_y_train = [y_train[i] for i in ran_list]
        x_train = pad_sequences(ran_x_train, self.TIME_STAMPS)
        y = pad_sequences(ran_y_train, self.TIME_STAMPS)
    ...
    def load_pretrained_embedding(self):
        ...
        with open(self.embedding_file, 'r', encoding='UTF-8') as f:
            words = f.read().strip().split('\t')[1:-1]
            leng_words = len(words)
            for i in range(math.floor(leng_words/2)):
            #for i in [24708]:
                values = words[2*i+1].replace('\n', ' ').replace('  ', ' ').replace(r'\n', '').replace('  ', ' ').replace('[ ', '[').replace('] ', ']').replace(' ', ',').replace(',,', ',').replace(',,', ',').replace(',]', ']').strip(str(i+1))
                ''''''
                ceshi = ''.join(values)
                print(ceshi)
                ff = open('ceshi.txt','w',encoding='UTF-8')
                ff.write(ceshi)
                ff.close()
                ''''''
